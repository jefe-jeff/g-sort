...
def load_vision_data(analysis_path: str,
    ...
    if include_ei:
        with EIReader(analysis_path,
            ...
            vcd.add_ei_from_loaded_ei_dict(eis_by_cell_id)
...
class VisionCellDataTable:
    ...
    def add_ei_from_loaded_ei_dict(self,
                                   ei_by_cell_id: Dict[int, 'EIContainer']) -> None:
        '''
...
class STAReader:
    ...
    def get_sta_for_cell_id(self,
        ...
        n_bytes_per_sta = STAReader.NUM_BYTES_PER_STIXEL * self.depth * self.height * self.width + \
                          self.depth * (N_BYTES_32BIT * 2 + N_BYTES_64BIT) + N_BYTES_64BIT + N_BYTES_32BIT
        sta_bytearray = self.sta_fp.read(n_bytes_per_sta)

        red_value = np.zeros((self.width, self.height, self.depth), dtype=np.float32)
        blue_value = np.zeros((self.width, self.height, self.depth), dtype=np.float32)
        green_value = np.zeros((self.width, self.height, self.depth), dtype=np.float32)

        red_error = np.zeros((self.width, self.height, self.depth), dtype=np.float32)
        blue_error = np.zeros((self.width, self.height, self.depth), dtype=np.float32)
        green_error = np.zeros((self.width, self.height, self.depth), dtype=np.float32)

        vcext.pack_sta_from_bytearray(sta_bytearray,
                                      self.width,
                                      self.height,
                                      self.depth,
                                      red_value,
