...
def gsort_artifacts(n, significant_electrodes, electrode_list, signals_tmp, iterations,max_iter, noise, data_on_cells, damping = 0.5, sat_band = 5, unsatured_min= -1000, unsaturated_max = 400, artifact_cluster_estimate_set = None):
    """
    Main run script
    """
    ...
    event_labels = np.arange(len(signals))
    ...
    if len(set(event_labels))==1 or iterations == 0:
        ...
        return 1, G, (event_labels, signals), (event_labels, signals), (event_labels, signals), {}, 0, note
    ...
    edge_cell_to_fit_info, edge_cell_to_signal, edge_cell_to_error, edge_cell_to_loss, edge_cell_to_noise_loss = find_candidate_edges(n, electrode_list, data_on_cells, difference_signals, event_signal_ordering,  mask, event_labels, noise, constrained = constrained, artifact_estimate = 0)
    
    sorted_edges = sorted(edge_cell_to_loss.items(), key=lambda item: item[1])
    
#     sorted_edges = [e for e in sorted_edges if (np.sum(edge_cell_to_signal[e[0]].flatten()**2)/np.sum(cell_eis[cell_ids.index(e[0][1])].flatten()**2) )>= 0.9]
...
#     print(sorted_edges)
    ...
    roots = [n for n in G.nodes if G.in_edges(n)==0]        
    if len(roots) > 1:
        note += "incomplete"
        for r in roots[1:]:
            event_labels[event_labels==r] = roots[0]
            
    return finished, G, (initial_event_labels, signals_tmp), (event_labels_with_virtual, signals), (event_labels, final_signals), edge_to_matched_signals, tracker, note

        
# def gsort_propagate(pattern, n, significant_electrodes, electrode_list, signals_tmp, iterations,max_iter, noise, data_on_cells, damping = 0.5, sat_band = 5, unsatured_min= -1000, unsaturated_max = 400, artifact_cluster_estimate_set = None):
#     """
#     Main run script
#     """
...
#     num_trials, num_electrodes, num_samples = signals.shape
#     noise_band = (num_samples-8)*2*sum([noise[e]**2 for e in electrode_list])
#     close_to_artifact = []
#     for artifact_cluster_estimate in artifact_cluster_estimate_set:
#         distance_from_artifact  = np.sum(signals[:,np.array(electrode_list)!=(pattern-1),8:].reshape((num_trials, -1))**2, axis = 1)-2*np.sum( ( signals[:,np.array(electrode_list)!=(pattern-1),8:]*artifact_cluster_estimate[:,np.array(electrode_list)!=(pattern-1),8:]).reshape((num_trials, -1)) , axis = 1) + np.sum(artifact_cluster_estimate[:,np.array(electrode_list)!=(pattern-1),8:]**2)
        
        
#         close_to_artifact += [any(distance_from_artifact<=noise_band)]
#     if any(close_to_artifact):
#         chosen_artifact = close_to_artifact.index(True)
#     else:
#         chosen_artifact = len(close_to_artifact)-1
    
        
    
#     artifact_cluster_estimate = artifact_cluster_estimate_set[chosen_artifact]
...
#     root_signals_ind = np.argwhere(distance_from_artifact<=noise_band).flatten()
#     root_signals_ind_opp = np.argwhere(distance_from_artifact>noise_band).flatten()
#     root_signals = np.vstack((artifact_cluster_estimate, signals[root_signals_ind])) if any(distance_from_artifact<=noise_band) else artifact_cluster_estimate
#     open_signals = signals[root_signals_ind_opp]
#     num_root_trials = len(root_signals)
#     num_opp_trials = len(open_signals)
    
    
#     artifact_cluster_estimate = np.mean(root_signals, axis = 0, keepdims =True)
#     cells_on_trials = {i:[] for i in range(num_trials)}
#     note = ""
#     if all((distance_from_artifact<=noise_band)):
#          # Create nodes for explanation graph
#         event_labels = np.zeros(num_trials)
#         G = nx.DiGraph()
#         G.add_nodes_from(list(set(event_labels)))
#         state_to_cells ={e:[] for e in event_labels}
...
#         # If all cliques have been merged, quit
#         if len(set(event_labels))==1 or iterations == 0:
#             note += "all signals matched to prev root"
#             return False, (artifact_cluster_estimate, chosen_artifact), 1, G, (event_labels, signals), (event_labels, signals), (event_labels, signals), {}, 0, note
    
#     if any((distance_from_artifact<=noise_band)):
# #         print(distance_from_artifact<=noise_band)
#         jump = False
#     else:
#         jump = True
...
#     # Generate mask to deal with saturation
#     signals_max = np.max(open_signals, axis = 0)
    
#     signals_min = np.min(open_signals, axis = 0)
#     mask = ((signals_max < unsaturated_max)*(signals_min > unsatured_min)).astype(np.float64)
#     mask_loc = np.argwhere(1-mask)
#     for m in mask_loc:
#         mask[m[0],max(0,m[1]-sat_band):min(m[1]+sat_band, num_samples)] = 0
...
#             try:
#                 event_labels = ap.fit_predict(open_signals[:, s_channels, :].reshape((num_opp_trials, -1)) )              
#                 break
#             except:
#                 pass
...
#     # 2. Cluster merging
#     while True:
#         event_labels = first_merge_event_cliques_by_noise(electrode_list,open_signals, event_labels,  mask, significant_electrodes,   noise)
...
#     event_labels = np.concatenate((np.zeros(num_root_trials), np.array(event_labels)+1))
    

    
#     signals = np.vstack((root_signals, open_signals))
...
#     # Find candidate edges in graph from comparing templates and difference signals
#     edge_cell_to_fit_info, edge_cell_to_signal, edge_cell_to_error, edge_cell_to_loss, edge_cell_to_noise_loss = find_candidate_edges_raw(n, electrode_list, data_on_cells, difference_signals, event_signal_ordering,  mask, event_labels, noise, constrained = constrained, artifact_estimate = 0)
...
#     return jump, (artifact_cluster_estimate, chosen_artifact), finished, G, (initial_event_labels, signals_tmp), (event_labels_with_virtual, signals), (event_labels, final_signals), edge_to_matched_signals, tracker, note
    ...
    #     print(signals_max, signals_min)
        ...
        with warnings.catch_warnings():
            ...
            for r_state in range(10):
                ...
                ap = sklearn.cluster.AffinityPropagation(damping=damping, random_state=r_state)
                try:
                    event_labels = ap.fit_predict(signals[:, s_channels, :].reshape((num_trials, -1)) )              
                    break
                except:
                    pass
...
# def spike_sorter_EA(n, significant_electrodes, electrode_list, signals_tmp, iterations,max_iter, noise, data_on_cells, damping = 0.5, sat_band = 5, unsatured_min= -1000, unsaturated_max = 400, artifact_cluster_estimate = None, clustering = "affinity propagation"):
#     """
#     Main run script
#     """
...
#     try: 
#         seed_electrode = min(np.argwhere(np.sum(1-mask, axis = 1) == 0)).item()
#         non_saturated_channels = list(np.argwhere(np.sum(1-mask, axis = 1) == 0))
#         s_channels = np.array([s for s in significant_electrodes if s in non_saturated_channels])
#     except:
#         note += "all relevant electrodes saturated"
#         s_channels = significant_electrodes
#     if clustering == "affinity propagation":
#         with warnings.catch_warnings():
#             warnings.simplefilter("error")
#             for r_state in range(10):
...
#                 event_labels = ap.fit_predict(signals[:, s_channels, :].reshape((num_trials, -1)) ) 
#     else:
#         X = signals[:, s_channels, :].reshape((num_trials, -1))
#         print("X", X.shape)
#         sil_vals = []
#         opt_config = []
#         for n_clusters in range(2,20+1):
#             r_states = []
#             for random_state in range(10+1):
                
#                 clusterer = sklearn.cluster.KMeans(n_clusters=n_clusters, random_state=random_state)
#                 cluster_labels = clusterer.fit_predict(X)


#                 silhouette_avg = sklearn.metrics.silhouette_score(X, cluster_labels)
#                 r_states += [silhouette_avg]
#             opt_r =  r_states[np.argmin(r_states)]
#             opt_config += [(n_clusters, np.argmin(r_states), opt_r)]
#         print("opt_config",opt_config)
#         earliest_change = list(np.diff(np.array([v for _, _, v in opt_config])) >= 0).index(True)
#         min_config = opt_config[earliest_change]
        
#         clusterer = sklearn.cluster.KMeans(n_clusters=min_config[0], random_state=min_config[1])
#         event_labels = clusterer.fit_predict(X)
#     # First unsupervised clustering of event cliques with noise-based event clique merging
#     old_event_cliques = [set([i for i, x in enumerate(event_labels == l) if x]) for l in set(event_labels)]
#     tries = 0
#     while True:
#         event_labels = first_merge_event_cliques_by_noise(electrode_list,signals, event_labels,  mask, significant_electrodes,   noise)
...
# #         event_labels = resort_clusters_by_noise(electrode_list,signals, event_labels,   mask, significant_electrodes,  noise)
#         event_cliques = [set([i for i, x in enumerate(event_labels == l) if x]) for l in set(event_labels)]
...
# #     event_labels = noise_ball_clustering(signals, mask, noise, electrode_list)
...
#     # Find candidate edges in graph from comparing templates and difference signals
#     edge_cell_to_fit_info, edge_cell_to_signal, edge_cell_to_error, edge_cell_to_loss, edge_cell_to_noise_loss = find_candidate_edges(n, electrode_list, data_on_cells, difference_signals, event_signal_ordering,  mask, event_labels, noise, constrained = constrained, artifact_estimate = artifact_cluster_estimate)
    
#     sorted_edges = sorted(edge_cell_to_loss.items(), key=lambda item: item[1])
#     _, _, num_samples = signals.shape
# #     baseline_variance =[np.log(noise[e]) + 1 for e in electrode_list]
# #     bump = sum([baseline_variance[e]*(num_samples-25) for e in significant_electrodes])
    

# #     sorted_edges = [(e,l) for e, l in sorted_edges if (edge_to_noise_loss[e]-bump)/(l-bump) > 1 ]
...
#         if not skip:
#             incoming_nodes = [tail]
#             incoming_edge_cells = [cell]
#             trial_signals = []
#             for u, v, ddict in G.in_edges(head, data=True):
#                 if cell != ddict['cell']:
#                     incoming_nodes += [u] 
#                     incoming_edge_cells += [ddict['cell']]
#                     sig = edge_cell_to_signal[((v, u),ddict['cell'])]
#                     shifts = get_shifts(signals,event_labels, v, u)                       
#                     shifted_sig = shift_sig(sig,shifts)
#                     trial_signals += [shifted_sig]

#             sig = edge_cell_to_signal[((head, tail), cell)]
#             shifts = get_shifts(signals,event_labels, head, tail)                       
#             shifted_sig = shift_sig(sig,shifts)
#             trial_signals += [shifted_sig]
...
#                 num_new_nodes = 2**(len(incoming_nodes)-1) - 1
#                 new_nodes = [i for i in range(len(G.nodes), len(G.nodes)+num_new_nodes)]
#                 for nn in new_nodes:
#                     downstream[nn] = incoming_nodes
#                 check_points_loss[(tail, head)] = loss
#                 check_points[(tail, head)] = (loss, G.copy(), copy.deepcopy(v_structure_edges_and_nodes), copy.copy(signals), copy.copy(event_labels), 
#                                              edge_to_cell_and_fit_info.copy(), edge_to_signal.copy(), edge_to_error.copy(), edge_to_loss.copy(), copy.deepcopy(sorted_edges), nodes_to_virtual_nodes.copy(), downstream.copy())
#                 v_structure_edges_and_nodes += [((tail, head), new_nodes)]
#     #             v_structure_edges_and_nodes += [((tail, head), [])]
# #                 powerset([i for i in range(len(incoming_nodes))])
#                 new_node_combinations = set(list(powerset([i for i in range(len(incoming_nodes))]))).difference(set(list(powerset([i for i in range(len(incoming_nodes)-1)]))))

#                 new_node_combinations = [nc for nc in new_node_combinations if len(nc)>1]

#                 virtual_signals = []
#                 virtual_event_labels = []
#                 for nl, nc in zip(new_nodes, new_node_combinations):
#                     tmp_signals = copy.copy(signals[event_labels==head])
# #                     [incoming_nodes[t] for t in nc]
# #                     [incoming_edge_cells[t] for t in nc]
#                     for t in nc:
#                         tmp_signals -= trial_signals[t]
#                     virtual_signals += [tmp_signals]
#                     virtual_event_labels += [nl for _ in range(len(tmp_signals))]

#                     downstream[nl] += [n for n, nnc in zip(new_nodes, new_node_combinations) if len(nnc) < len(nc)]

#                 virtual_event_labels = np.array(virtual_event_labels, dtype = int)
#                 virtual_signals = np.concatenate(virtual_signals, axis = 0) if num_new_nodes > 1 else virtual_signals[0]





#                 #relevant_other_nodes = list(initial_event_set) + nodes_to_virtual_nodes[head] if head in nodes_to_virtual_nodes.keys() else list(initial_event_set)
#                 relevant_other_nodes = incoming_nodes + nodes_to_virtual_nodes[head] if head in nodes_to_virtual_nodes.keys() else incoming_nodes

#                 relevant_other_trials = np.array([i for i, l in enumerate(event_labels) if l in relevant_other_nodes])
#                 ds_tmp, eso_tmp = compute_difference_signals_from_virtual(signals[relevant_other_trials], virtual_signals, event_labels[relevant_other_trials], virtual_event_labels)
#                 etcfi_tmp, ets_tmp, ete_tmp, etl_tmp,etnl_tmp = find_candidate_edges(n, electrode_list, data_on_cells, ds_tmp, eso_tmp, mask, np.append(event_labels[relevant_other_trials], virtual_event_labels), noise, constrained = constrained, artifact_estimate = artifact_cluster_estimate)
#                 chosen_directions = [(y, x) for (x, y) in etcfi_tmp.keys()]
#                 for (ux, vy) in chosen_directions:
#                     if vy in downstream.keys():
#                         if ux in downstream[vy]:

#                             loss, G,v_structure_edges_and_nodes,signals,event_labels, edge_to_cell_and_fit_info, edge_to_signal, edge_to_error, edge_to_loss, sorted_edges, nodes_to_virtual_nodes, downstream=check_points[(tail, head)]
#                             G.remove_edge(tail, head)
#                             skip = True
#                             break
#                 if not skip:
#                     edge_cell_to_fit_info.update(etcfi_tmp)
#                     edge_cell_to_signal.update(ets_tmp)
#                     edge_cell_to_error.update(ete_tmp)
#                     edge_cell_to_loss.update(etl_tmp)
#                     edge_cell_to_noise_loss.update(etnl_tmp)

#                     if num_new_nodes > 1:
#                         # Compute difference signals
#                         ds_tmp, eso_tmp =compute_difference_signals(virtual_signals, np.array(virtual_event_labels))

#                         # Clean the difference signals
#                         ds_tmp = clean_signals(ds_tmp)

#                         # Find candidate edges in graph from comparing templates and difference signals
#                         etcfi_tmp, ets_tmp, ete_tmp, etl_tmp, etnl_tmp = find_candidate_edges(n, electrode_list, data_on_cells, ds_tmp, eso_tmp, mask, virtual_event_labels, noise, constrained = constrained, artifact_estimate = artifact_cluster_estimate, plot = False)
...
#                             edge_cell_to_noise_loss.update(etnl_tmp)
#                     if not skip:
#                         signals = np.append(signals, virtual_signals, axis = 0)

#                         event_labels = np.append(event_labels, virtual_event_labels)

#                         sorted_edges = sorted(edge_to_loss.items(), key=lambda item: item[1])
#                         chosen_edges = list(G.edges)
#                         sorted_edges = [(x, y) for (x, y) in sorted_edges if (x[1], x[0]) not in chosen_edges]

#                         sorted_edges = [(e,l) for e, l in sorted_edges if (edge_to_noise_loss[e]-bump)/(l-bump) > 1 ]


#                         if head in nodes_to_virtual_nodes.keys():
#                             nodes_to_virtual_nodes[head] += new_nodes
#                         else:
#                             nodes_to_virtual_nodes[head] = new_nodes

#                         G.add_nodes_from(new_nodes)
#             else:
#                 out_nodes = [out_edge[1] for out_edge in G.out_edges(tail) if out_edge[1]!=head]
#                 illegal_edges = [(o, head) for o in out_nodes]+[(head,o) for o in out_nodes]

#                 sorted_edges = [(e, l) for e, l in sorted_edges if e[0] not in illegal_edges]
#                 sorted_edges = [(e, l) for e, l in sorted_edges if e[0] != (head, tail)]


            

#         problem_edges, resolved_v_structure_nodes = check_graph(G)

#         if len(problem_edges) > 0:
#             problem_nodes = [a for e in problem_edges for a in e]
            
#             problem_vstructures = []
#             for pb_i, (pb_edges, pb_nodes) in enumerate(v_structure_edges_and_nodes):
          
#                 if any([(pb_n in problem_nodes) for pb_n in pb_nodes]) or (pb_edges in problem_edges):
#                     problem_vstructures += [pb_i]
            
#             v_structure_edges = [v_structure_edges_and_nodes[pb_i][0] for pb_i in problem_vstructures]
            
        
#             bad_vstructure = v_structure_edges[np.argmin([check_points_loss[ve] for ve in v_structure_edges])]
            
#             loss, G,v_structure_edges_and_nodes,signals,event_labels, edge_to_cell_and_fit_info, edge_to_signal, edge_to_error, edge_to_loss, sorted_edges, nodes_to_virtual_nodes, downstream=check_points[bad_vstructure]
            
#             G.remove_edge(bad_vstructure[0], bad_vstructure[1])
#             problem_edges, resolved_v_structure_nodes = check_graph(G)
            
#         else:
#             v_structure_edges_and_nodes_tmp = []
#             for e_star, n_star_list in v_structure_edges_and_nodes:
#                 if any([len(list(nx.all_simple_paths(G, n_star, e_star[1]))) <= 1 for n_star in n_star_list]):
#                         v_structure_edges_and_nodes_tmp += [(e_star, n_star_list)]
#             v_structure_edges_and_nodes = v_structure_edges_and_nodes_tmp
...
#         if len(sorted_edges) == 0 and not finished and len([x for x,_ in v_structure_edges_and_nodes]) > 0:
#             unresolved_v_structure_nodes = [n for n in G.nodes if n not in resolved_v_structure_nodes and G.in_degree(n) > 1]
#             unresolved_v_structure_edges = [x for x, y_ in v_structure_edges_and_nodes for y in y_ if G.in_degree(y) == 0 and G.out_degree(y)==0]
             
            
#             if len(unresolved_v_structure_nodes) != 0:
#                 unresolved_v_structure_edges = [e for n in unresolved_v_structure_nodes for e in G.in_edges(n)]
#                 unresolved_v_structure_edges = list(set(unresolved_v_structure_edges).intersection(set([x for x,_ in v_structure_edges_and_nodes])))


#                 bad_vstructure = unresolved_v_structure_edges[np.argmin([check_points_loss[ve] for ve in unresolved_v_structure_edges])]
#                 loss, G,v_structure_edges_and_nodes,signals,event_labels, edge_to_cell_and_fit_info, edge_to_signal, edge_to_error, edge_to_loss, sorted_edges, nodes_to_virtual_nodes, downstream=check_points[bad_vstructure]
                
#                 G.remove_edge(bad_vstructure[0], bad_vstructure[1])
#                 problem_edges, resolved_v_structure_nodes = check_graph(G)
#             elif len(unresolved_v_structure_nodes) == 0 and len(unresolved_v_structure_edges) != 0:
...
#                 problem_edges, resolved_v_structure_nodes = check_graph(G)
#             else:
#                 bad_vstructure = v_structure_edges_and_nodes[np.argmax([check_points_loss[ve] for ve, _ in v_structure_edges_and_nodes])][0]
#                 loss, G,v_structure_edges_and_nodes,signals,event_labels, edge_to_cell_and_fit_info, edge_to_signal, edge_to_error, edge_to_loss, sorted_edges, nodes_to_virtual_nodes, downstream=check_points[bad_vstructure]
...
# #     identical_nodes = resolve_duplicate_signals(G)
# #     edge_to_snr = {}
# #     for (x, y) in G.edges:
# #         edge_to_snr[(y, x)] = (edge_to_noise_loss[(y, x)]-bump)/(edge_to_loss[(y, x)]-bump)

# #     identical_nodes = [e for e, s in edge_to_snr.items() if s <1]
    
# #     while len(identical_nodes) != 0:
# #         ns = identical_nodes.pop()
# #         new_label = min(ns)
# #         old_label = max(ns)
# #         G = nx.contracted_nodes(G, new_label, old_label)
# #         identical_nodes = [(x, new_label) if y == old_label else (new_label, y) if x == old_label else (x,y) for (x, y) in identical_nodes]
# #         edge_to_signal = {((x, new_label) if y == old_label else (new_label, y) if x == old_label else (x,y)):z for (x, y), z in edge_to_signal.items() }
# #         event_labels = [new_label if l==old_label else l for l in event_labels]
...
# #     if not finished:
# #         event_labels = find_friends(electrode_list, signals, initial_event_labels,  mask, significant_electrodes,  noise, G)
...
# #     if iterations > 0:
# #         valid_graph = check_valid_graph(n, final_g)
#     edge_to_matched_signals = {}
...
#         threshold_array = (np.ones((num_events, num_events)) - np.eye(num_events)) * noise_threshold * 2
...
#             X = signals[event_labels == event_set[node_pair[0]]] * mask[None,:,:]
#             Y = signals[event_labels == event_set[node_pair[1]]] * mask[None,:,:]

#             Z = np.einsum('ijk->jik',all_pairwise(X,Y))
#             num_diff_signals = Z.shape[1]
#             distance[node_pair[0], node_pair[1]] = np.sum(Z[significant_electrodes]**2)/num_diff_signals
#             distance[node_pair[1], node_pair[0]] = np.sum(Z[significant_electrodes]**2)/num_diff_signals
        
        
#         # Find all edges whose distance is less than the threshold
#         edges = np.argwhere((distance < threshold_array) *(distance != 0))
#         if len(edges) == 0:
#             break

#         # Find the shortest edge (if it is below threshold, and contract it)
...
def first_merge_event_cliques_by_noise(electrode_list, signals, event_labels_tmp,  mask, significant_electrodes, noise):
    ...
    _, _, num_samples = signals.shape
#     baseline_variance =[noise[e]**2 for e in electrode_list]
#     std = np.repeat([noise[e] for e in electrode_list], num_samples)
...
#             threshold_array[node_pair[0], node_pair[1]] += 2 * np.sqrt(2 * k / len(X) /len(Y))
#             threshold_array[node_pair[1], node_pair[0]] += 2 * np.sqrt(2 * k / len(X) /len(Y))

            #variable se 
#             threshold_array[node_pair[0], node_pair[1]] += 1.96 * np.sqrt(2/num_diff_signals) * k
#             threshold_array[node_pair[1], node_pair[0]] += 1.96 * np.sqrt(2/num_diff_signals)* k


        
        # Find all edges whose distance is less than the threshold
#         print("distance", distance)
#         print("threshold", threshold_array)
        edges = np.argwhere((distance < threshold_array) *(distance != 0))
        if len(edges) == 0:
            break
#         print(threshold_array)
...
def spike_sorter_EA_eval(n, significant_electrodes, electrode_list, signals_tmp, iterations,max_iter, noise, data_on_cells, damping = 0.5, sat_band = 5, unsatured_min= -1000, unsaturated_max = 400, artifact_cluster_estimate = None):
    """
    Main run script
    """
...
# def spike_sorter_EA_experiment(n, significant_electrodes, electrode_list, signals_tmp, iterations,max_iter, noise, data_on_cells, damping = 0.5, sat_band = 5, unsatured_min= -1000, unsaturated_max = 400, artifact_cluster_estimate = None):
#     """
#     Main run script
#     """
...
#             earliest_change = list(np.diff(np.array([v for _, _, v in opt_config])) >= 0).index(True)
#         except:
#             earliest_change = len(opt_config)-1
#         min_config = opt_config[earliest_change]
