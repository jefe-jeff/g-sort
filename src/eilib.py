...
corrColDict['k'] = 'k'
corrColDict['soma'] = '#1f77b4'
corrColDict['axon'] = '#d62728'
corrColDict['dendrite'] = '#17becf'
corrColDict['mixed'] = '#bcbd22'
corrColDict['error'] = 'k'

ctColDict = {}
ctColDict['on parasol'] = '#1f77b4'
ctColDict['off parasol'] = '#2ca02c'
ctColDict['on midget'] = '#d62728'
ctColDict['off midget'] = '#bcbd22'
...
def plotEiStim(ei, stim_elecs, ax, coords, size_fac=-25,thr=-1,stim_sf=240,stim_mew=3,ei_color='b',stim_color='y',alpha=1,label=None,plotArray=True):
    #Data
    #coords = loadCoords512()
    xs = coords[:,0]
    ys = coords[:,1]
...
def plotEiThr(eiElecInds, eiAmps, ax, coords, size_fac=-25,ei_color='b',alpha=1,label=None,plotArray=True,squared=False):
    xs = coords[:,0]
    ys = coords[:,1]
    eixs = coords[eiElecInds,0]
    eiys = coords[eiElecInds,1]
    ##Plot elecs
    if plotArray:
        ax.scatter(xs, ys, facecolors='k', edgecolors='k', s=5,alpha=.5)
    ##Plot EI
    if squared:
        ax.scatter(eixs,eiys, facecolors=ei_color, edgecolors=ei_color, s=size_fac*-(eiAmps**2), alpha=alpha,label=label)
    else:
        ax.scatter(eixs,eiys, facecolors=ei_color, edgecolors=ei_color, s=size_fac*eiAmps, alpha=alpha,label=label)
    ##Cosmetics
    ax.tick_params(axis='x',which='both',bottom=False,top=False,labelbottom=False)
    ax.tick_params(axis='y',which='both',left=False,right=False,labelleft=False)
    ax.set_xlim([min(xs)-100, max(xs)+100])
...
def plotEiColsThr(eiElecInds, eiAmps, eiWaveforms, ax, coords, size_fac=-25,alpha=1,label=None,plotArray=True,squared=False,uppBound=1.6,lowBound=0.05):
    xs = coords[:,0]
    ys = coords[:,1]
    eixs = coords[eiElecInds,0]
    eiys = coords[eiElecInds,1]
    ...
    ##Plot EI
    ###get colors
    colors = [corrColDict[axonorsomaRatio(w,uppBound=uppBound,lowBound=lowBound)] for w in eiWaveforms]
    if squared:
        ax.scatter(eixs,eiys, facecolors=colors, edgecolors=colors, s=size_fac*-(eiAmps**2), alpha=alpha,label=label)
    else:
        ax.scatter(eixs,eiys, facecolors=colors, edgecolors=colors, s=size_fac*eiAmps, alpha=alpha,label=label)
    ##Cosmetics
    ax.tick_params(axis='x',which='both',bottom=False,top=False,labelbottom=False)
    ax.tick_params(axis='y',which='both',left=False,right=False,labelleft=False)
    ax.set_xlim([min(xs)-100, max(xs)+100])
...
def plotEiFrame(eiFrame,thr,ax,coords,size_fac=-25,myfc='b',noArray=False, myalpha=1):

    #Plot array
    if not noArray:
        ax.scatter(coords[:,0], coords[:,1], facecolors='k', edgecolors='k', s=2.5,alpha=.5,zorder=0)
    #Plot EI
    goodInds = np.nonzero(eiFrame<thr)[0]
    mys = eiFrame[goodInds]*size_fac
    im = ax.scatter(coords[goodInds,0],coords[goodInds,1],
            facecolors=corrColDict[myfc], edgecolors='none',s=mys,alpha=myalpha,zorder=1)

    ##Cosmetics
    ax.tick_params(axis='x',which='both',bottom=False,top=False,labelbottom=False)
    ax.tick_params(axis='y',which='both',left=False,right=False,labelleft=False)
    ax.set_xlim([min(coords[:,0])-100, max(coords[:,0])+100])
    ax.set_ylim([min(coords[:,1])-100, max(coords[:,1])+100])
    ax.axis('off')
...
def plotEiColsHollowThr(eiElecInds,eiAmps,fig,ax,coords,size_fac=-25,cmap=cm.jet,edgecolor='k',noArray=False,myalpha=1,set_size=1,cbarLab='',cbarLabSize=22,labelpad=20,clip=False,hSize=100,yClip=False,lims=(0,0,0,0),xBound=0,lw=1):
    xs = coords[:,0]
    ys = coords[:,1]
    eixs = coords[eiElecInds,0]
    eiys = coords[eiElecInds,1]
    ...
    if set_size == 1:
        ...
        mys = np.sqrt(eiAmps*size_fac)
    else:
        mys = set_size

    if not xBound:
        im = ax.scatter(eixs,eiys, facecolors='none', edgecolors=edgecolor,s=mys,alpha=myalpha,zorder=1,lw=lw)
    else:
        im = ax.scatter([i for i in eixs if i > xBound],[i for ii,i in enumerate(eiys) if eixs[ii] > xBound],
                facecolors='none', edgecolors=edgecolor,s=[i for ii,i in enumerate(mys) if eixs[ii] > xBound], alpha=myalpha,zorder=1,lw=lw)
...
def plotArrayOnlyCols(fig,ax, coords, cols, cmap=cm.jet, size=50, myalpha=1,cbarLab='',vmin=None,vmax=None,cbarFlg=True,linewidths=5,zorder=None):
    xs = coords[:,0]
    ys = coords[:,1]
    im = ax.scatter(xs, ys, c=cols, s=size, alpha=myalpha, cmap=cmap,vmin=vmin,vmax=vmax,linewidths=linewidths,zorder=zorder)
    ax.set_xlim([min(xs)-100, max(xs)+100])
    ax.set_ylim([min(ys)-100, max(ys)+100])
    ax.set_xticks([])
    ax.set_yticks([])
...
def getSomaLocThr(somaElecs,somaWeights,elec_coords):

    weightlist = somaWeights/sum(somaWeights) #normalize
    soma_elec_coords_x = [elec_coords[j,0] for j in somaElecs]
    soma_elec_coords_y = [elec_coords[j,1] for j in somaElecs]
    #Compute average position
    xcoord = np.dot(soma_elec_coords_x, weightlist)
    ycoord = np.dot(soma_elec_coords_y, weightlist)
...
def getSomaLocWaveRatioLeanThr(cell_df,elec_coords):
    #compartments already computed in df
    cell_df_soma = cell_df[cell_df['compartment']=='soma']

    #Compute centroid
    weightlist = np.array(cell_df_soma['eiVal']/sum(cell_df_soma['eiVal']))
    #Get cell coordinates
    soma_elec_coords_x = [elec_coords[j,0] for j in cell_df_soma['elecInd']]
    soma_elec_coords_y = [elec_coords[j,1] for j in cell_df_soma['elecInd']]
    #Compute average position
    xcoord = np.dot(soma_elec_coords_x, weightlist)
    ycoord = np.dot(soma_elec_coords_y, weightlist)
...
def getAxonFitThr(allElecs, allWeights, somaLoc, elec_coords, pdeg = 4, distThr = 175):

    c = elec_coords

    #Get list of cell elec coordinates
    elecinds = [j for j in allElecs if getLocDist(c[j,:],somaLoc) > distThr]
    cell_coords_x = [c[j,0] for j in elecinds]
    cell_coords_y = [c[j,1] for j in elecinds]
    weights = [allWeights[jj] for jj,j in enumerate(allElecs) if getLocDist(c[j,:],somaLoc) > distThr]
    ...
    p = np.poly1d(np.polyfit(fitx, fity, pdeg, w=weights))
    pder = np.polyder(p)

    return(p, pder, dirflg, fitx, np.array(elecinds)) #also return x coords in range
...
def loadNoise(noisepath):
    ...
    with open(noisepath,'r') as f:
        for ll,line in enumerate(f.readlines()):
            if ll == 0: continue #ignore TTL channel
            noises.append(float(line.strip()))
    return noises
...
def plotEiBalloonThr(cell_df, ax, elec_coords, size_fac=-8,ei_color='b',alpha=1,label=None,plotArray=True,axonClipFlg = True,axonFlg='',textFlg=False,bcFlg=False,fontsize=20,linewidth=5,foreground='w'):

    warnings.filterwarnings('ignore')

    ax.tick_params(axis='x',which='both',bottom=False,top=False,labelbottom=False)
    ax.tick_params(axis='y',which='both',left=False,right=False,labelleft=False)
    ax.set_xlim([min(elec_coords[:,0])-100, max(elec_coords[:,0])+100])
    ax.set_ylim([min(elec_coords[:,1])-100, max(elec_coords[:,1])+100])

    if plotArray:
        ax.scatter(elec_coords[:,0],elec_coords[:,1],c='k',s=25) #stim elecs

    badCellFlg = False

    ##plot somas
    somaElecs = np.array(cell_df[cell_df['compartment']=='soma']['elecInd'])
    somaWeights = np.array(cell_df[cell_df['compartment']=='soma']['eiVal'])
    somaLoc = getSomaLocThr(somaElecs,somaWeights,elec_coords)
    if not ei_color:
        ei_color = np.random.rand(3,)
    if np.any(somaElecs):
        ax.scatter(somaLoc[0],somaLoc[1],marker='o',facecolors='none',s=size_fac*np.min(somaWeights),edgecolors=ei_color,lw=linewidth)
    else: #has no soma elecs
        badCellFlg = True

    ##plot axons
    #axonElecs = np.array(cell_df[cell_df['compartment']=='axon']['elecInd'])
    #axonWeights = np.array(cell_df[cell_df['compartment']=='axon']['eiVal'])
    (p,pder,dirflg,fitx,fitElecs) = getAxonFitThr(np.array(cell_df['elecInd']), np.array(cell_df['eiVal']), somaLoc, elec_coords, pdeg = 4, distThr = 10)
    if p:
        fitx = np.linspace(np.sort(fitx)[0],np.sort(fitx)[-1],50)
        if axonClipFlg:
            if axonFlg == 'd':
                fitx = fitx[fitx<somaLoc[1]]
        if dirflg == 'x':
            ax.plot(fitx,p(fitx),lw=3,color=ei_color,alpha=.6)
        elif dirflg == 'y':
            ax.plot(p(fitx),fitx,lw=3,color=ei_color,alpha=.6)
    else:
        badCellFlg = True

    if textFlg:
        if not badCellFlg:
            txt = ax.text(*somaLoc,str(cell_df['cell'].iloc[0]),color=ei_color,fontsize=fontsize)
            txt.set_path_effects([PathEffects.withStroke(linewidth=linewidth, foreground=foreground)])
            txt.set_alpha(alpha)
    return cell_df['cell'].iloc[0] if badCellFlg else 0


def plotEiTextThr(cell_df, ax, coords, size_fac=-12,ei_color='b',alpha=1,label=None,plotArray=True,foreground='w',squared=False,fontsize=20,linewidth=5):

    somapos = getSomaLocWaveRatioLeanThr(cell_df,coords)
    if not ei_color:
        ei_color = np.random.rand(3,)
    plotEiThr(cell_df['elecInd'], cell_df['eiVal'], ax, coords,size_fac=size_fac,ei_color=ei_color,alpha=alpha,label=label,plotArray=plotArray,squared=squared)
    txt = ax.text(*somapos,str(cell_df['cell'].iloc[0]),color=ei_color,fontsize=fontsize)
    txt.set_path_effects([PathEffects.withStroke(linewidth=linewidth, foreground=foreground)])
    txt.set_alpha(alpha)

def loadPaths(paths_mat='/Volumes/Lab/Users/Sasi/pystim/scripts/datasets_se_reference/datasets_list_se.mat', mat_name = 'datinfo'):
    paths = {}
    foo = loadmat(paths_mat)[mat_name]
    paths['pieces'] = [i[0] for i in foo['datasets'][0][0][0]]
    paths['eipaths'] = [i[0] for i in foo['datasets_ei'][0][0][0]]
    paths['excludedPieces'] = [i-1 for i in foo['excluded'][0][0][0]] #adjust indexing
    paths['stimpaths'] = [i[0][0][0] if i[0][0] else '' for i in foo['folders'][0][0][0]]

    #added for manual datasets
    paths['eipaths_manual'] = [i[0] if i.size>0 else '' for i in foo['datasets_ei_manual'][0][0][0]]
    paths['stimpaths_manual'] = [i[0][0][0] if i[0][0].size>0 else '' for i in foo['folders_manual'][0][0][0]]
